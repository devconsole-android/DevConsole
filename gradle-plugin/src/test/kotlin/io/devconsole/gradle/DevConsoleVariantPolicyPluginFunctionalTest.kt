...
class DevConsoleVariantPolicyPluginFunctionalTest {
    ...
    fun `auto-wired coordinate resolves to the released non-snapshot version`() {
        ...
        // coordinate is 1.0.0 -- the DEFAULT_SDK_VERSION must not point at a 1.0.0-SNAPSHOT that was
        ...
        assertTrue(result.output, result.output.contains("io.github.devconsole-android:devconsole-noop:1.0.0"))
        assertTrue(result.output, !result.output.contains("1.0.0-SNAPSHOT"))
    }
    ...
    fun `declaring only an add-on coordinate still auto-wires the core runtime`() {
        writeFixture(
            ...
            extraBuildScript =
                """
                dependencies {
                    add("debugImplementation", "io.github.devconsole-android:devconsole-ui-compose:1.0.0")
                }
                """.trimIndent(),
        )
        ...
        assertTrue(result.output, result.output.contains("io.github.devconsole-android:devconsole:1.0.0"))
